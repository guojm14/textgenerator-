...
def TPS_STN(U, nx, ny, out_size):
    ...
    def _solve_system1(cp, nx, ny):
        ...
        for _ in range(ny):
          ...
          cy += gy
        p_=p_/3
        offsetx=2*np.random.random(2)+1
    ...
    cp = tf.constant(v.reshape([1, nx*ny, 2]), dtype=tf.float32)
    T, fp, cp = _solve_system1(cp, nx, ny)
    
    
    output1 = _transform(T, fp, U, out_size)
    cp=cp[:,1:]
    print cp
    cp=tf.constant(cp.reshape([1, nx*ny, 2]), dtype=tf.float32)
    T, fp= _solve_system(cp, nx, ny)
    output2 = _transform(T, fp, output1, out_size)
